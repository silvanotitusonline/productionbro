-- Reconstructed non-production baseline contract.
--
-- Source recovery note (2026-08-26): this migration was reconstructed from read-only
-- catalogue inspection of the approved isolated Supabase project `eqwstpdjoineycrkhpht`.
-- It is not a claim about production history and MUST NOT be applied to any environment
-- that has already recorded `recover_hardened_support_case_persistence`.
--
-- Enforcement model:
--   * community_cases and case_messages deny direct client access under RLS;
--   * authenticated users use narrowly scoped SECURITY DEFINER RPCs only;
--   * residents can access their own cases, Case Staff only cases assigned to them,
--     and System Administrators only through the existing verified-MFA helper;
--   * private helpers have no client execute privilege and every mutation is audited.
...
create table public.community_cases (
  ...
  state public.case_state not null default 'OPEN'::public.case_state,
  ...
  constraint community_cases_title_check
    check (char_length(trim(title)) between 3 and 180),
  constraint community_cases_category_check
    check (char_length(trim(category)) between 3 and 120),
  constraint community_cases_description_check
    check (char_length(trim(description)) between 3 and 10000),
  constraint community_cases_priority_check
    check (priority between 1 and 5),
  constraint community_cases_location_label_check
    check (location_label is null or char_length(trim(location_label)) between 2 and 180)
);
...
create table public.case_messages (
  ...
  created_at timestamptz not null default now(),
  constraint case_messages_body_check
    check (char_length(trim(body)) between 1 and 10000)
);

-- These names are intentionally retained because the later performance-cleanup migration
-- removes them after its composite-index replacement is available.
create index community_cases_resident_idx on public.community_cases(resident_id, updated_at desc);
...
create policy community_cases_no_direct_client_access
  on public.community_cases for all
  using (false)
  with check (false);
...
begin
  ...
  insert into public.community_cases(
    ...
    v_resident, v_title, v_category, v_description, coalesce(p_priority, 3), v_location,
...
begin
  ...
  if char_length(v_body) not between 1 and 10000
