...
-- `community_profiles.handle` is unique. A fixed `anonymous` handle allowed only
-- one guest to bootstrap; later anonymous posts failed before their draft existed.
-- Keep the display name anonymous while deriving an opaque, stable per-session handle.
...
declare
  v_display_name text;
  v_avatar_path text;
  v_anonymous_handle text := 'anonymous_' || replace(left(p_user_id::text, 12), '-', '');
begin
  ...
  if p_user_id = auth.uid() and private.is_anonymous_guest() then
    ...
    values (
      p_user_id, 'Anonymous', v_anonymous_handle, null, '',
